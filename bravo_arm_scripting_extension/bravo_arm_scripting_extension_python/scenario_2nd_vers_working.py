...
class BravoRmpFlowExampleScript:
    ...
    def load_example_assets(self):
        ...
        #path_to_robot_usd = "/home/ashes/isaacsim/custom_robot_usd_files/bravo/bravo_script_ext_vers.usd"
        ...
        #self._articulation = SingleArticulation(robot_prim_path + "/root_joint")

        # NOTE: this little block of code is no longer necessary, since bravo_script_ext_vers.usd has the 
        # initial joint positions set to good values.
        # set initial joint positions to avoid self-collisions on scene init: 
        #joint_names = ["bravo_axis_a", "bravo_axis_b", "bravo_axis_c", "bravo_axis_d", "bravo_axis_e", "bravo_axis_f", "bravo_axis_g"]
        #joint_initial_values = np.array([0.0, 3.14159, 3.14159, 0.0, 3.14159, 3.14159, 3.14159, 0.0, 0.0])
        #self._articulation.set_joint_positions(joint_initial_values)
        #self._articulation.set_joints_default_state(positions=joint_initial_values)
        ...
        self._goal_block = DynamicCuboid(
            ...
            #position=np.array([0.35, -0.2, 0.025]),
        ...
        # i'm loading in my own stage that already has a ground plane, so i don't need the next line: 
    ...
    def my_script(self):
        ...
        success = yield from self.goto_position(
            ...
            #translation_target, orientation_target, self._articulation, self._rmpflow, timeout=500
        )
        ...
        #lower_translation_target = np.array([0.4, 0, 0.155]) # z-pos: too low: 0.1, 0.15 (just barely); too high: 0.2, 0.16 (just a bit)
        ...
        #yield from self.close_gripper_bravo(self._articulation, close_position=np.array([0.02, 0.02]), atol=0.006)
        #yield from self.close_gripper_bravo(self._articulation, close_position=np.array([0.0]), atol=0.05)
        #yield from self.close_gripper_bravo(self._articulation, close_position=np.array([4.4]), atol=0.05)
        ...
        #high_translation_target = np.array([0.4, 0, 0.6])
        #high_translation_target = np.array([0.4, -0.2, 0.575])
        #high_translation_target = np.array([0.4, -0.2, 0.4])
        #high_translation_target = np.array([0.6, -0.2, 0.4])
        #high_translation_target = np.array([0.5, -0.2, 0.5])
        #high_translation_target = np.array([0.5, -0.2, 0.4])
        #high_translation_target = np.array([0.5, -0.2, 0.5])
        #high_translation_target = np.array([0.4, -0.2, 0.575])
        """
        high_translation_target = np.array([0.4, -0.2, 0.7])
        high_orientation_target = euler_angles_to_quats([0, 0, -0.4363])
        self._target.set_world_pose(high_translation_target, high_orientation_target)
        ...
        #high_translation_target = np.array([0.4, -0.2, 0.575])
        #high_translation_target = np.array([0.4, -0.2, 0.5])
        high_left_translation_target = np.array([0.4, -0.2, 0.55])
        #high_orientation_target = euler_angles_to_quats([0, 0, -0.4363])
        #high_orientation_target = euler_angles_to_quats([0, 0.2618, -0.5236])
        #high_orientation_target = euler_angles_to_quats([0, 0.5236, -0.5236])
        #high_orientation_target = euler_angles_to_quats([0, 0.6109, -0.5236])
        #high_left_orientation_target = euler_angles_to_quats([0, 0.4363, -0.4363])
        #high_left_orientation_target = euler_angles_to_quats([0, 0.0, -0.4363])
        #high_left_orientation_target = euler_angles_to_quats([0, 0.4, -0.4363])
        #high_left_orientation_target = euler_angles_to_quats([0, 0.39, -0.4363])
        #high_left_orientation_target = euler_angles_to_quats([0, 0.38, -0.44])
        #high_left_orientation_target = euler_angles_to_quats([0, 0.375, -0.43])
        #high_left_orientation_target = euler_angles_to_quats([0, 0.37, -0.45])
        #high_left_orientation_target = euler_angles_to_quats([0, 0.36, -0.45])
        ...
        #next_translation_target = np.array([0.4, 0.4, 0.6])
        #next_translation_target = np.array([0.4, 0.2, 0.575])
        #next_translation_target = np.array([0.5, 0.2, 0.5])
        #next_translation_target = np.array([0.5, 0.2, 0.4])
        #next_translation_target = np.array([0.5, 0.2, 0.5])
        #next_translation_target = np.array([0.4, 0.2, 0.575])
        #next_translation_target = np.array([0.4, 0.2, 0.5])
        high_right_translation_target = np.array([0.4, 0.2, 0.55])
        #next_orientation_target = euler_angles_to_quats([0, 0.2618, 0.4363])
        #high_right_orientation_target = euler_angles_to_quats([0, 0.4363, 0.4363])
        ...
        next_translation_target = np.array([0.5, 0.2, 0.25])
