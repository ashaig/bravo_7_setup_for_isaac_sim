...
class FrankaRmpFlowExampleScript:
    ...
    #def get_usd_from_urdf(self, root_path, file_name):
    def get_usd_from_urdf(self, root_path, file_name):
        ...
        import_config.make_default_prim = False #True  # Make the robot the default prim in the scene
        import_config.self_collision = True #False  # Disable self-collision for performance
        ...
        # Import the robot onto the current stage and retrieve its usd path
        result, usd_path = omni.kit.commands.execute(
            "URDFImportRobot",
    ...
    def load_example_assets(self):
        ...
        urdf_root_path = "/home/ashes/isaacsim/custom_urdf_files/bravo_description/urdf"
        urdf_file_name = "bravo.urdf"

        #path_to_robot_usd = self.get_usd_from_urdf(urdf_root_path, urdf_file_name)
        robot_prim_path = "/World/bravo_manipulation"
        #robot_usd, robot_prim_path = self.get_usd_from_urdf()
        #path_to_robot_usd = robot_usd.get_assets_root_path()

        # Do not reload assets when hot reloading.  This should only be done while extension is under 
        # development.
